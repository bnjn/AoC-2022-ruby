...
unused_space = 70000000 - fs_map['/'][:size]

result = []

fs_map.each do |dir, info|
    if info[:size] >= (30000000 - unused_space)
        result.push(info[:size])
    end
end

puts 'Min file size to delete in order to make room for update: '
pp result.sort.first

unused_space += result.sort.first

puts 'Enough space for update? ' + (unused_space >= 30000000).to_s
